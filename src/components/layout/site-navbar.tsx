...
function SiteNavbarContent({ navItems, contactLink, pathname }: SiteNavbarProps & { pathname: string }) {
  ...
  function handleServiceNavigation(event: ReactMouseEvent<HTMLAnchorElement>, href: string) {
  ...
  return (
    ...
    >
      <header
        ref={headerRef}
        className="bg-brand-white text-brand-black w-full px-5 shadow-md max-lg:**:text-sm sm:px-10 lg:px-13"
      >
              ...
              {serviceItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    onClick={(event) => handleServiceNavigation(event, item.href)}
      ...
      {navigatingTo && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-white/75 backdrop-blur-[2px]" role="status" aria-live="polite">
          <div className="flex flex-col items-center gap-3 rounded-2xl border border-black/10 bg-white px-8 py-6 shadow-xl">
            <LoaderCircle className="text-brand-burgundy size-8 animate-spin" aria-hidden="true" />
            <span className="font-raleway text-brand-black text-sm font-medium">Loading service...</span>
          </div>
        </div>
      )}
    </div>
