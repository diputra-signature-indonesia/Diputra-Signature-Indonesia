      ...
      <section className="rounded-xl border border-[#C8CDD5] bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] sm:p-6">
        ...
        <div className="mt-5 space-y-3">
          {documents.map((document) => <article key={document.id} className="flex items-center gap-2 rounded-lg border border-[#C8CDD5] bg-[#F2F4F7] transition hover:border-[#9FA8B5] hover:bg-[#ECEFF3]">
            {document.signedUrl ? <a href={document.driveUrl ?? document.signedUrl} target="_blank" rel="noreferrer" className="flex min-w-0 flex-1 items-center justify-between gap-4 rounded-lg px-4 py-3.5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8C1010]/20"><FileSummary document={document} /></a> : <div className="flex min-w-0 flex-1 items-center justify-between gap-4 px-4 py-3.5"><FileSummary document={document} /></div>}
