  ...
  return <>
    ...
    <AdminModal open={open} onClose={() => { if (!isPending) setOpen(false); }} title="Edit Job" description="Perubahan akan tersimpan pada Job ini. Status diubah terpisah pada Job Information." size="lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        {detail.canChangePic ? <div><label htmlFor="edit-job-pic" className={labelClass}>PIC *</label><ProfileCombobox id="edit-job-pic" value={form.picId} options={profiles} onChange={(profileId) => setField('picId', profileId)} placeholder="Search PIC..." /></div> : <p className="text-xs text-[#68717E]">PIC: {detail.summary.pic}. Hanya admin yang dapat mengganti PIC.</p>}
