...
export async function getJobDocuments(jobId: string) {
  ...
  const [folderResult, documentsResult] = await Promise.all([
    supabase
      ...
      .select('id,job_id,google_drive_id,google_folder_id,folder_name,web_view_url,connection_status,last_synced_at,version')
      .eq('job_id', jobId)
      .is('archived_at', null)
      .maybeSingle(),
    supabase
      .from('job_documents')
      .select('id,job_id,google_file_id,file_name,mime_type,file_size_bytes,web_view_url,sync_status,uploaded_at,uploaded_by,version')
      .eq('job_id', jobId)
      .is('archived_at', null)
